  ...
  float pcktlossRate2 = 0;
    ...
    for (auto iter = stats.begin (); iter != stats.end (); ++iter) {
      ...
      if(counter%2 == 0){
          ...
          pcktlossRate1 = pcktlossRate1 + (iter->second.lostPackets)*100.0/iter->second.txPackets;
      }
        
      if(counter%2 == 1){
          Throughput2 = Throughput1 + throughputKbps ; 
          pcktlossRate2 = pcktlossRate2 + (iter->second.lostPackets)*100.0/iter->second.txPackets;
      }
    ...
    pcktlossRate1=pcktlossRate1/counter;
    pcktlossRate2=pcktlossRate1/counter;

    NS_LOG_UNCOND("Throughput1 = "<< Throughput1<< "Kbps");
    NS_LOG_UNCOND("Throughput2 = " << Throughput2<< "Kbps");
    NS_LOG_UNCOND("pcktloss1 = "<< pcktlossRate1<< "%");
    NS_LOG_UNCOND("pcktloss2 = " << pcktlossRate2<< "%");

    writer1<< bottleNeckLinkDataRate/10000<<" "<<Throughput1 <<std::endl;
    writer2<< bottleNeckLinkDataRate/10000<<" "<<Throughput2 <<std::endl;
    writer3<< pcktlossRate1<<" "<<Throughput1 <<std::endl;
    writer4<< pcktlossRate2<<" "<<Throughput2 <<std::endl;
